...
Page({
  ...
  onLoad: function(options) {
    ...
    let noForgetText1 = wx.getStorageSync('noForgetText')
    console.log(noForgetText1)
    var tagEle = []
    var row ={}

    //截取标题
    for (var i = 0; i < noForgetText1.length;i++){
      var text = (noForgetText1[i].area).slice(0, 9)
      row.title = text
      tagEle.push(row)
      row={}
    }
  ...
  textInfoTap:function(e){
    ...
    db.collection('text').where({
      ...
      success: res => {
        wx.setStorageSync('deleId', res.data[0]._id)
        that.setData({
          flag2:false,
          forgetText:res.data[0].area,
          forgetDate: util.formatTime(res.data[0].createTime),
        })
      }
    })
  },
